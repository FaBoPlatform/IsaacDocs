...
class UIBuilder:
    ...
    def _on_set_object(self):
        """Cubeを追加して剛体を設定し、反射のないクリスタルのような見た目を適用"""
        ...
        # 反射のないクリスタルのような色情報を追加
        material_path = Sdf.Path("/World/Materials/NonReflectiveCrystalMaterial")
        ...
        print(f"Cube {cube.GetPath()} に剛体と反射のないクリスタルのような赤色、10%の透過率が追加されました")
