...
class UIBuilder:
    ...
    def reset_xform_stack(self, prim):
        """
        Resets the XformStack for the given prim to avoid issues with rigid body hierarchies.
        """
        xformable = UsdGeom.Xformable(prim)
        if xformable:
            xformable.ClearXformOpOrder()
            xformable.AddTranslateOp().Set(Gf.Vec3f(0.0, 0.0, 1.0))  # Example transform
            print(f"XformStack reset for {prim.GetPath()}")

    def _on_set_object(self):
        """
        Adds a Robot object to the USD scene, sets position, scale, rotation, and configures rigid body physics and collision.
        Fixes hierarchy issues by resetting XformStack.
        """
        # Get USD stage context
        ...
        # Create a new prim in the current layer
        prim_spec = Sdf.CreatePrimInLayer(current_layer, prim_path)

        # Load Robot USD file as payload
        prim_spec.payloadList.Prepend(Sdf.Payload(
            'E:/isaac_sim/isaac-sim-4.0.0/MyExtentions/MyTest/data/fabo_jetdog/fabo_jetdog2.usd', 
            Sdf.Path.emptyPath))

        # Get the Robot prim
        robot_prim = stage.GetPrimAtPath(prim_path)
        
        self.RL_hip_joint = UsdPhysics.DriveAPI.Get(stage.GetPrimAtPath("/World/Robot/jetdog/base_link/RL_hip_joint"), "angular")
        self.RR_hip_joint = UsdPhysics.DriveAPI.Get(stage.GetPrimAtPath("/World/Robot/jetdog/base_link/RR_hip_joint"), "angular")
        self.FL_hip_joint = UsdPhysics.DriveAPI.Get(stage.GetPrimAtPath("/World/Robot/jetdog/base_link/FL_hip_joint"), "angular")
        self.FR_hip_joint = UsdPhysics.DriveAPI.Get(stage.GetPrimAtPath("/World/Robot/jetdog/base_link/FR_hip_joint"), "angular")
        self.RL_thigh_joint = UsdPhysics.DriveAPI.Get(stage.GetPrimAtPath("/World/Robot/jetdog/rl_top_1/RL_thigh_joint"), "angular")
        self.RR_thigh_joint = UsdPhysics.DriveAPI.Get(stage.GetPrimAtPath("/World/Robot/jetdog/rr_top_1/RR_thigh_joint"), "angular")
        self.FL_thigh_joint = UsdPhysics.DriveAPI.Get(stage.GetPrimAtPath("/World/Robot/jetdog/fl_top_1/FL_thigh_joint"), "angular")
        self.FR_thigh_joint = UsdPhysics.DriveAPI.Get(stage.GetPrimAtPath("/World/Robot/jetdog/fr_top_1/FR_thigh_joint"), "angular")
        self.RL_calf_joint = UsdPhysics.DriveAPI.Get(stage.GetPrimAtPath("/World/Robot/jetdog/rl_middle_1/RL_calf_joint"), "angular")
        self.RR_calf_joint = UsdPhysics.DriveAPI.Get(stage.GetPrimAtPath("/World/Robot/jetdog/rr_middle_1/RR_calf_joint"), "angular")
        self.FL_calf_joint = UsdPhysics.DriveAPI.Get(stage.GetPrimAtPath("/World/Robot/jetdog/fl_middle_1/FL_calf_joint"), "angular")
        self.FR_calf_joint = UsdPhysics.DriveAPI.Get(stage.GetPrimAtPath("/World/Robot/jetdog/fr_middle_1/FR_calf_joint"), "angular")
        
        # Set drive parameters
        self.set_drive_parameters(self.RL_hip_joint, "position", 0, stiffness=1000, damping=100, max_force=500)
        self.set_drive_parameters(self.RR_hip_joint, "position", 0, stiffness=1000, damping=100, max_force=500)
        self.set_drive_parameters(self.FL_hip_joint, "position", 0, stiffness=1000, damping=100, max_force=500)
        self.set_drive_parameters(self.FR_hip_joint, "position", 0, stiffness=1000, damping=100, max_force=500)
        self.set_drive_parameters(self.RL_thigh_joint, "position", 0, stiffness=1000, damping=100, max_force=500)
        self.set_drive_parameters(self.RR_thigh_joint, "position", 0, stiffness=1000, damping=100, max_force=500)
        self.set_drive_parameters(self.FL_thigh_joint, "position", 0, stiffness=1000, damping=100, max_force=500)
        self.set_drive_parameters(self.FR_thigh_joint, "position", 0, stiffness=1000, damping=100, max_force=500)
        self.set_drive_parameters(self.RL_calf_joint, "position", 0, stiffness=1000, damping=100, max_force=500)
        self.set_drive_parameters(self.RR_calf_joint, "position", 0, stiffness=1000, damping=100, max_force=500)
        self.set_drive_parameters(self.FL_calf_joint, "position", 0, stiffness=1000, damping=100, max_force=500)
        self.set_drive_parameters(self.FR_calf_joint, "position", 0, stiffness=1000, damping=100, max_force=500)
